...
브라우저에서 `http://localhost:18080` 또는 `http://localhost:18080/fortune-app.html` 로 접속합니다.
...
| 서비스 | URL | 설명 |
|--------|-----|------|
| 🔮 **운세 앱 (메인)** | http://localhost:18080/fortune-app.html | **사주팔자, 운세, 토정비결 GUI** |
| 🏠 홈페이지 | http://localhost:18080 | 랜딩 페이지 |
| 📚 API 문서 | http://localhost:18080/api/docs | REST API 문서 |
| 🧪 API 테스트 | http://localhost:18080/api/docs/test | 개발자용 API 테스트 |
| 📅 간지달력 | http://localhost:18080/api/calendar/view/current | 이번 달 간지달력 |
...
- ✅ **AI 영수증 운영 정책** — 승인된 운영 90일 retention과 보호된 Actuator 기간·도메인 집계 추가(개발 기본 비활성)
- ✅ **Synthetic AI canary gate** — 개인정보 없는 고정 fixture, `fixture+engine` 성공 영수증 기반 운영 startup 1회 실행, 동시 실행 차단, 모델 원문 비노출
- ✅ **AI 엔진 캐시 v4** — 출생시각 정규화 계약 변경에 맞춰 engine/AI cache namespace 승격

### v3.1.2 (2026-07-28)
- ✅ **결정론 엔진 우선 계약** — 사주·일일·별자리·토정비결 엔진 결과를 버전이 있는 `fortune-fact-packet/v1`으로 AI 서술 계층에 전달
- ✅ **AI no-override 검증** — 일간·일주·종합 점수·괘 번호가 엔진 결과와 충돌하면 외부 응답을 폐기하고 로컬 규칙 기반 해석으로 전환
...
- ✅ 토정비결 144괘(상8×중6×하3) 결정론 산식으로 교체, 일일운세·신살·간지달력 난수 제거
- ✅ JDK 17/25 혼재 → **21로 통일** (build.gradle·Dockerfile·CI/CD)
- ✅ `docs/reference/` 01~12 개발·사용자·아키텍처 가이드 신규
- ✅ 코드리뷰(Copilot·ultrareview) 지적 반영, 전체 33 테스트 통과
...
- **🏠 메인 홈페이지**: http://localhost:18080
- **📚 API 문서**: http://localhost:18080/api/docs
- **🧪 API 테스트**: http://localhost:18080/api/docs/test
- **📅 간지달력 보기**: http://localhost:18080/api/calendar/view/current
...
# 브라우저에서 확인
open http://localhost:18080/api/docs
```

### API 테스트
```bash
# 인터랙티브 테스트 페이지
open http://localhost:18080/api/docs/test
```
