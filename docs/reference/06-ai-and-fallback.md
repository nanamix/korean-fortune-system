...
| 기본 제외 필드 | 이름, 생년월일시, 출생 위치·시간대, 보정 일시, 역법, 성별, 알림 대상 |

사주 패킷에는 사주팔자·일간·일주·오행·십신·대운·세운·월운처럼 이미 계산된 결과만 포함합니다. 일일·별자리·토정비결도 점수, 점수 근거, 행운 요소, 월별 결과 등 엔진 출력만 포함합니다. 별자리 패킷은 계산 모델·정밀도, Sun·Moon·Rising, 원소·양상·데칸·달 위상, 주요 transit·orb·가감값을 전달하지만 원본 출생일시·좌표·시간대는 전달하지 않습니다.

별자리용 프롬프트는 패킷에 없는 행성·하우스·각을 만들지 못하게 하고, 출생 차트 핵심 성향 → 대상일 transit → 관계·일·건강·재정 → 점수 근거 → 행동 3가지 → 한계 순서로 서술하도록 제한합니다. LLM은 계산을 담당하지 않고 결정론 엔진 결과를 설명합니다.
